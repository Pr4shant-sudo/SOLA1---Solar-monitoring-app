            ...
            <button key={n.id} onClick={() => setScreen(n.id)} style={{
              ...
              {screen === n.id && (
                <div style={{
                  ...
                  transform: "translateX(-50%)",
                  width: 28, height: 3, borderRadius: 2, background: COLORS.green,
                }} />
              )}
              <span style={{ fontSize: 20 }}>{n.icon}</span>
              <span style={{ fontSize: 9, color: screen === n.id ? COLORS.green : COLORS.textDim, fontWeight: screen === n.id ? 700 : 400 }}>
                {n.label}
              </span>
            </button>
          ))}
